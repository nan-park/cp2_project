...
start = time.time()
s_bert = model_builder.load_sbert(CFG.MODEL_PATH, CFG.SBERT_MODEL_FOLDER)
timestamp = datetime.datetime.now().strftime("%Y.%m.%d-%H:%M:%S")
...
if not vectorized_file.is_file():
    timestamp = datetime.datetime.now().strftime("%Y.%m.%d-%H:%M:%S")
    print(f"[INFO] ({timestamp}) No Vectorized numpy array. Vectorize text data...")
...
save_path = f"{CFG.MODEL_PATH}/{now}"

modelckpt_callback = tf.keras.callbacks.ModelCheckpoint(
    f"{save_path}/{model_name}.hdf5",
    monitor='val_loss',
    save_best_only=True,
...
print(f"[INFO] ({timestamp}) Model saved '{save_path}/{model_name}.hdf5'")